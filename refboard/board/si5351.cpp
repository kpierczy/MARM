...
{
	const auto res =  m_bus.write(SI5351_BUS_BASE_ADDR, &addr, sizeof addr, data, bytes );
	if( res ) {
		dbg_err( "I2C bus transaction failed %i", res );
	}
	return res;
#if 0
...
{
	const auto res =  m_bus.write(SI5351_BUS_BASE_ADDR, &addr, sizeof addr, &data, sizeof data );
	if( res ) {
		dbg_err( "I2C bus transaction failed %i", res );
	}
	return res;
...
#endif
	const auto res = m_bus.transfer( SI5351_BUS_BASE_ADDR, &addr, sizeof addr,
			&reg_val, sizeof reg_val );
	if( res ) {
		dbg_err( "I2C bus rd transaction failed %i", res );
	}
